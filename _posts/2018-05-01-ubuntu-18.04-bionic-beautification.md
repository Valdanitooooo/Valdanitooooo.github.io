...
我安装了的主要有以下几个：
User Themes ：从用户目录加载 shell themes
Dash to Dock : 可以修改左侧菜单栏的一些样式
Dash to Panel ： 这个厉害了，可以把左侧菜单栏和顶部状态栏合并到下面显示，类似windows效果
Arc Menu ： 全新的应用菜单，类似win7内种
...
安装这款主题需要先安装GTK+3.0
  ...
  ```
在～/.local/share目录下新建themes、icons和fonts目录，分别用于存放主题、图标和字体文件，将之前下载的文件解压后拷贝到对应目录即可，我是直接命令行安装的，参考整个主题的[readme](https://github.com/vinceliuice/matcha){:target="_blank"}。
